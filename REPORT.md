...
- ``main.py``: This file contains the main code to train and test the agent from the command line using Argparse. It is possible to train the agent from scratch or load a pre-trained model. The code is also able to evaluate the agent in the environment in different modes, e.g., visual and non-visual mode. A small adjustment has been made to ``main.py`` in contrast to the continuous control project, namely that we process states for both players and that we have to calculate actions for both players. We therefore learn more efficiently by using the experiences of both players.

- ``src/agents.py:`` Contains the implementation of the different actor-critic agents. At the moment, only the DDPG agent is available. Compared to the continuous control project, the agent restricts the actions to a range of $[-1,1]$ by using ``np.clip()`` in the method ``choose_action``. Following agents are available at the moment:
    - ``DDPGAgent``
...
The ``ActorNetwork`` ( $\mu(s|\theta^{\mu})$ ) maps states to actions and therefore tries to directly learn a policy. It is a simple three-layer fully connected network with Rectified Linear Unit (ReLU) activations, Batch Normalization (in our implementation, it is LayerNorm though) and Adaptive Moment Estimation (Adam) optimizer which is an extension of stochastic gradient descent. The input layer is fully-connected/dense and has 24 nodes for the 24 features with 400 output nodes. The first layer is followed by Batch Normalization (LayerNorm) and after that followed by a ReLU activation. This layer-structure is repeated one more time where the second fully-connected/dense layer has 400 input nodes and 300 output nodes and is also followed by Batch Normalization and then with a ReLU activation. The third fully-connected/dense layer (called $\mu$ ) has 300 input nodes and 2 output nodes for every possible action. The output of the last layer is followed by an hyperbolic tangent (tanh) activation to restrict the output into a range of [-1,1]. The network is implemented in ``src/networks.py``.
...
The input layer is fully-connected/dense and has 24 nodes for the 24 features with 400 output nodes. The first layer is followed by Batch Normalization (LayerNorm) and after that followed by a ReLU activation. 
After that, the second fully-connected/dense layer follows with 400 input nodes and 300 output nodes. This layer is followed by Batch Normalization (LayerNorm) again but without the ReLU activation.
The third fully-connected/dense layer (you can also call it the second hidden-layer) has 2 input nodes for every possible action and 300 output nodes. This layer does not take any pre-processed input from the other layers but represents the point where the action is finally fed into the CriticNetwork given that the authors of the DDPG paper state: *"Actions were not included until the 2nd hidden layer of Q."*
We therefore end up with a state-value given by the output of the last second fully-connected layer and with an action-value calculated by the third fully-connected layer (second hidden layer). Both the state- and the action-value have 300 entries and are then combined by addition which results in an state-action-value. This combined state-action-value is fed into the last fully-connected/dense layer which has 300 input nodes and one single output node that represent the Q-value of this state-action-value.
...
The DDPG agent was able to solve the collaboration and competition problem with an average score of 0.5+ over 100 consecutive episodes after 648 episodes. See the following plots for the training and evaluation of the agent.

### DDPG

Training plot of the **DDPG algorithm**: 

![DDPG-Training](plots/Collaboration_and_competition_project_DDPGAgent_train.png)


Evaluation of the trained **DDPG agent** over 30 episodes with deterministic behavior:

![DDPG-Evaluation](plots/Collaboration_and_competition_project_DDPGAgent_eval.png)
