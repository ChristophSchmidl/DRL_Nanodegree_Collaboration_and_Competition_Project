...
def random_exploration(env):
    ...
    unity_env = env.reset(train_mode=False)[brain_name]
...
if __name__ == '__main__':
    ...
    print_device_info()
    env = get_env(visual_mode=True)
    print_env_info(env)
    
    # TennisBrain
    action_size = 2 # Vector Action space size (per agent): 2
    state_size = 24 # Vector Observation space size (per agent): 8
